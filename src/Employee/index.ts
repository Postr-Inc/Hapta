...
export const pb = new Pocketbase("https://postly.pockethost.io");
...
switch (true) { 
  case !Bun.env.EmployeeEmail || !Bun.env.EmployeePassword:
    console.error({
      message:
        "Please set the EmployeeEmail and EmployeePassword in your config file, if you do not know your password ask your administrator!",
...
try {
  await pb.collection("Postly_Staff_Auth").authWithPassword(
    Bun.env.EmployeeEmail,
    Bun.env.EmployeePassword,
    {
...
const PUBLIC_WHITELIST = ["/auth/check"];

if (limiter) {
  app.use('*', async (c, next) => {
    const path = c.req.path;

    // Skip rate limiter for whitelisted paths
    if (PUBLIC_WHITELIST.includes(path)) {
      return next();
    }
...
app.post("/deepsearch", async (c) => {
 ...
 app.post("/collection/:collection", async (c) => {
  ...
  if (contentType.includes("multipart/form-data")) {
    ...
    security = JSON.parse(form.get("security")?.toString() || "{}");
